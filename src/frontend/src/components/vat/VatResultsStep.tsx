...
export default function VatResultsStep({
  ...
  const label = result.message || 'Standard VAT';
  const isReverseCharge = label === 'Reverse Charge';
  const isExempt = label === 'Exempt';
  const isZeroRatedExport = label === 'Zero Rated Export';
  const isZeroRated = label === 'Zero Rated';
  const isReduced = label === 'Reduced VAT';
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center py-2 border-t pt-3">
              <span className="text-sm text-muted-foreground">VAT Treatment:</span>
          ...
          {isZeroRatedExport && (
            <Alert className="bg-green-50 border-green-200 dark:bg-green-950 dark:border-green-800">
              ...
              <AlertDescription className="ml-2 text-green-900 dark:text-green-100">
                ...
                <p className="text-sm">
                  This is a zero-rated export. VAT rate is 0%.
                </p>
                <p className="text-xs mt-2 opacity-80">
                  Zero-rated supply under applicable VAT legislation.
                </p>
              </AlertDescription>
            </Alert>
          )}

          {/* Zero Rated (UK domestic) */}
          {isZeroRated && !isZeroRatedExport && (
            <Alert className="bg-green-50 border-green-200 dark:bg-green-950 dark:border-green-800">
              <Info className="h-4 w-4 text-green-600 dark:text-green-400" />
              <AlertDescription className="ml-2 text-green-900 dark:text-green-100">
                <p className="font-medium mb-1">Zero Rated</p>
                <p className="text-sm">
                  This is a zero-rated supply. VAT rate is 0%.
                </p>
                <p className="text-xs mt-2 opacity-80">
          ...
          {isReverseCharge && (
            <Alert className="bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800">
              ...
              <AlertDescription className="ml-2 text-blue-900 dark:text-blue-100">
                ...
                <p className="text-sm">
                  Reverse charge applies – customer to account for VAT.
                </p>
                <p className="text-xs mt-2 opacity-80">
          ...
          {isExempt && (
            <Alert className="bg-amber-50 border-amber-200 dark:bg-amber-950 dark:border-amber-800">
              ...
              <AlertDescription className="ml-2 text-amber-900 dark:text-amber-100">
                ...
                <p className="text-sm">
                  This transaction is VAT exempt. Your invoice must include a legal note explaining the exemption basis.
                </p>
                <p className="text-xs mt-2 opacity-80">
          ...
          {isReduced && (
            <Alert className="bg-blue-50 border-blue-200 dark:bg-blue-950 dark:border-blue-800">
              <Info className="h-4 w-4 text-blue-600 dark:text-blue-400" />
              <AlertDescription className="ml-2 text-blue-900 dark:text-blue-100">
                <p className="font-medium mb-1">Reduced VAT Rate</p>
                <p className="text-sm">
                  Reduced VAT rate applied as per national VAT law.
                </p>
              </AlertDescription>
            </Alert>
          )}
